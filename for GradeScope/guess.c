...
void initalizeArray(void){ // why do i always have to initalize things inorder for them to work, why cant they just work...
	for(int i =0; i<5;i++){
...
void updateArray(Player champ){
	...
	for(int i=0;i<5;i++){ 
		...
		if(scoreBoard[i].score > playerScore){ //finds an index where the champ, belongs
			index = i;
			break;
		}
	}
...
void insertPlayer(Player champ){	
  ...
  //lowest score means the highest guess a player made.
	...
	if(playersInScoreBoard ==5 && champ.score > lowestScore){return;} // array is full, and player made more guesses than the highest guess score, we ignore it
